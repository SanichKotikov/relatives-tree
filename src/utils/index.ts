...
export const toMap = <T extends { id: any }>(items: ReadonlyArray<T>): Map<T['id'], T> => (
  new Map(items.map((item) => [item.id, { ...item }]))
);

export const hasDiffParents = (node: IFamilyNode): boolean => node.parents.map(prop('type')).filter(unique).length > 1;
export const byGender = (target: Gender) => (a: IFamilyNode, b: IFamilyNode) => (b.gender !== target) ? -1 : 0;
