...
export const prop =
  ...
  (item: T): T[K] =>
    item[name];
export const withId =
  <T extends { id: any }, K extends keyof T>(id: T[K]) =>
  (item: T) =>
    item.id === id;

export const withIds =
  <T extends { id: any }, K extends keyof T>(ids: readonly T[K][], include = true) =>
  (item: T) =>
    ids.includes(item.id) === include;
