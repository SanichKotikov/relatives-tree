...
const calcConnectors = (family: Family) => (connectors: Connector[], unit: Unit) => {
  ...
  const cX =
    getUnitX(family, child) + child.nodes.findIndex(withIds(getChildIDs(unit))) * SIZE + HALF_SIZE;
...
export const parents = (families: readonly Family[]): readonly Connector[] =>
  families.filter(withType(FamilyType.parent)).reduce<Connector[]>((connectors, family) => {
