...
const getChildIDs = (unit: Unit): readonly string[] => unit.nodes.map(prop('children')).flat().map(prop('id'));

const calcConnectors = (family: Family) => (connectors: Connector[], unit: Unit) => {
...
export const parents = (families: readonly Family[]): readonly Connector[] =>
  families
    ...
    .reduce<Connector[]>(
      (connectors, family) => connectors.concat(family.parents.reduce(calcConnectors(family), [])),
      [],
    );
